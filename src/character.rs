...
impl Characters {
    ...
    pub fn generate_characters_list(&self, content: &str) -> CharactersList {
        let cleaned_sentence = self.remove_punctuation_from_sentence(content, &self.punctuation);
...
mod tests {
    ...
    fn expect_to_return_chinese_char_list() {
        ...
        let handler = Characters::new().unwrap();
        let res = handler.generate_characters_list(content);
